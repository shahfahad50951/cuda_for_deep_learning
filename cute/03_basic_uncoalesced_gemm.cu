/*
    ...
    GPU Layouts used in this implementation for A and C matrix are non-optimal for 
...
int main(){
    ...
    auto ALayoutGPU = make_layout(make_shape(M, K), make_stride(K, 1));  // K-major
    auto BLayoutGPU = make_layout(make_shape(K, N), make_stride(1, K));  // K-major
    auto CLayoutGPU = make_layout(make_shape(M, N), make_stride(N, 1));  // N-major
...
/*
    Compilation Command:
    nvcc -lineinfo -I ./cutlass/include --std=c++17 -o basic_uncoalesced_gemm 03_basic_uncoalesced_gemm.cu 
*/